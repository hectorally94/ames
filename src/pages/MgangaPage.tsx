...
import { MissionDto, useGetMgangaByLanguageQuery } from '../redux/api/mgangaApiSlice';
import ComponentMganga from '../common/ComponentMganga';
...
const MgangaPage: React.FC = () => {
  ...
  //const { data, isLoading, error } = useGetAboutUsByLanguageQuery(i18n.language);
    ...
    const { data, isLoading,isFetching, error } = useGetMgangaByLanguageQuery(i18n.language, {
      skip: !i18n.language, // Skip query if no language is available
    });
  const { data: defaultData } = useGetMgangaByLanguageQuery('French'); // Fetch default language data (English)

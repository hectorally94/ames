...
const actionsApiSlice = apiServices
  ...
  .injectEndpoints({
    ...
    getPaginatedLastActionsAllOne: builder.query<PaginatedResponse<ActionDto>, { languageName?: string;clanguageName?: string; page?: number; size?: number; sort?: string }>({
      query: ({ languageName = '',clanguageName='',page = 0, size = 3, sort = 'id,desc' }) => ({
        url: '/findAllLastactionsAll',
     ...
     // Endpoint to fetch paginated actions with optional title filtering
     getPaginatedActionsAllOne: builder.query<PaginatedResponse<ActionDto>, { languageName?: string;clanguageName?: string;typeActionsTranslate?: string;categoryTranslate?: string; page?: number; size?: number; sort?: string }>({
      query: ({ languageName = '',clanguageName='', typeActionsTranslate = '',categoryTranslate='',page = 0, size = 12, sort = 'id,desc' }) => ({
        url: '/actionsAllPageByLangTypeTranslateLangCategoryTranslate',
    ...
    getPaginatedActions: builder.query<PaginatedResponse<ActionDto>, { title?: string; page?: number; size?: number; sort?: string }>({
      ...
      getActionById: builder.query<Action, number>({
        ...
        providesTags: (_result, _error, id) => [{ type: ACTION_TAGS[0], id }], // Tag for individual action
      }),
...
export const {
  ...
  useGetPaginatedActionsAllOneQuery,
  useGetPaginatedLastActionsAllOneQuery,
